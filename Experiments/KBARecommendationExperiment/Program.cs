...
namespace ML.KBARecommendation.ClassificationExperiment
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var dataView = mlContext.Data.LoadFromTextFile<DataInput>("train_tickets_kba.csv", separatorChar: ';', hasHeader: true, allowQuoting: true);

            var total = dataView.GetRowCount();

            Console.WriteLine("Loading Data into Memory");

            var incidentsData = mlContext.Data.CreateEnumerable<DataInput>(dataView, reuseRowObject: false).ToArray();
        ...
        static void TrainColumn(string labelColumn, DataInput[] incidentsData)
        {
            Console.CursorTop++;
            ...
            var fullData = mlContext.Data.LoadFromEnumerable<DataInput>(incidentsData);
            ...
            var config = new CsvConfiguration(CultureInfo.CurrentUICulture, delimiter: ";");
