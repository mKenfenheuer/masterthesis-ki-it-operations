...
namespace ML.IncidentClassification.ClassificationExperiment
{
    class Program
    {
        private static string TrainingDataRelativePath = $"data_train.csv";
        private static string TestDataRelativePath = $"data_test.csv";
        ...
        static void Main(string[] args)
        {
            var labelColumns = new string[] {
                nameof(IncidentInput.Category),
                //nameof(IncidentInput.BusinessService),
                nameof(IncidentInput.Urgency),
                nameof(IncidentInput.Impact),
                nameof(IncidentInput.TicketType),
            };
            ...
            string connectionString = @"Server=192.168.100.12;Database=ticket-data-ml;User Id=ticket-data;Password=%?%eX*!Gx^W0kNTxh{4zI0g>((;";

            string sqlCommand = @"SELECT [SysId]
                                      ,[Number]
                                      ,[ShortDescription]
                                      ,[LongDescription]
                                      ,[Category]
                                      ...
                                      ,[BusinessService]
                                      ,[TicketLanguage]
                                  FROM [ticket-data-ml].[dbo].[IncidentsEng]";

            DatabaseSource dbSource = new DatabaseSource(SqlClientFactory.Instance, connectionString, sqlCommand);

            var dataView = mlContext.Data.LoadFromTextFile<IncidentInput>("incident_data.csv", separatorChar: ';', hasHeader: true);

            var total = dataView.GetRowCount();

            Console.WriteLine("Loading Data into Memory");

            var incidentsData = mlContext.Data.CreateEnumerable<IncidentInput>(dataView, reuseRowObject: false).ToArray();
        ...
        static void TrainColumn(string labelColumn, IncidentInput[] incidentsData)
        {
            Console.CursorTop++;
            ...
            var split = mlContext.Data
                .TrainTestSplit(mlContext.Data.LoadFromEnumerable<IncidentInput>(incidentsData), testFraction: 0.1);

            var trainingDataView = split.TrainSet;
            var testDataView = split.TestSet;

            var train = split.TrainSet.GetRowCount();
            var test = split.TestSet.GetRowCount();
            ...
            var dataProcessPipeline = mlContext.Transforms.Conversion.MapValueToKey("Label", labelColumn)
                                         .Append(mlContext.Transforms.Concatenate("Features",
                                                     nameof(IncidentInput.Title), nameof(IncidentInput.Body)))
            ...
            {
                string modelRelativeLocation = $"..\\..\\..\\Models\\{labelColumn}_itsm_{trainer.name}Model.zip";
                ...
                var engine = mlContext.Model.CreatePredictionEngine<IncidentInput, IncidentPrediction>(trainedModelTransformer);
                TestResult result = ConsoleHelper.GetMultiClassClassificationTestMetrics(trainer.name.ToString(),
                    mlContext.Data.CreateEnumerable<IncidentInput>(testDataView, reuseRowObject: false).ToArray(),
                    labelColumn,
                    engine,
                    metrics);

                ConsoleHelper.GetConfusionMatrix(metrics).Save($"..\\..\\..\\Models\\Confusion\\{labelColumn}_itsm_{trainer.name}_confusion_matrix.png");
                ConsoleHelper.GetNormalizedConfusionMatrix(metrics).Save($"..\\..\\..\\Models\\Confusion\\{labelColumn}_itsm_{trainer.name}_normalized_confusion_matrix.png");
